...
def my_sections(
    ...
    for s in sections:
        assignments = (
            db.query(models.Assignment)
            .filter(models.Assignment.section_id == s.id)
            .all()
        )
...
def run_full_detection(text: str, domain: str, reference_text: str = None):
    # 1. Teacher similarity
    teacher_similarity = 0.0
    if reference_text:
        teacher_similarity = check_plagiarism(text, [reference_text])

    # 2. Public corpus hybrid similarity
    fingerprint_score, _ = compare_with_public_corpus(text, domain)

    corpus_texts = [item["text"] for item in PUBLIC_CORPUS.get(domain, [])]

    semantic_score = semantic_similarity(text, corpus_texts)
    char_score = char_ngram_similarity(text, corpus_texts)
    ...
    final_similarity = domain_adjusted_score(final_similarity, domain)

    # 3. Source Mapping
    source_mapping = semantic_source_mapping(text, domain)
    matched_phrases = extract_matched_phrases(text, domain)

    # 4. AI Detection (Hybrid)
    raw_ai_conf = hybrid_ai_score(text)
    ai_conf = stabilize_ai_score(raw_ai_conf, text)
    ai_label = "AI Generated" if ai_conf > 0.6 else "Human Written"

    return {
        "teacher_similarity": teacher_similarity,
        "public_similarity": final_similarity,
        "final_similarity": max(teacher_similarity, final_similarity),
...
async def check_personal_document(
    ...
    # ---------- plagiarism (hybrid ensemble) ----------
    similarity_score, total_fingerprints = compare_with_public_corpus(text, domain)
    matched_phrases = extract_matched_phrases(text, domain)

    corpus_with_sources = []

    for item in PUBLIC_CORPUS.get(domain, []):
       corpus_with_sources.append({
           "source": item.get("source", domain),
           "text": item.get("text", "")
    })
   
    corpus_texts = [item["text"] for item in corpus_with_sources]
    ...
    similarity_score = domain_adjusted_score(final_score, domain)
    sentence_analysis = sentence_similarity_analysis(text, domain)
    ...
    return [
        {
            ...
            "matched_phrases": r.matched_phrases or [],
            "ai_highlighted_phrases": [
                s["sentence"]
                for s in (r.sentence_analysis or [])
                if s.get("is_ai_generated") is True
        ],
